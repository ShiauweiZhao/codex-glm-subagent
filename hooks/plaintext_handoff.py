...
def state_lock(root: pathlib.Path):
    if fcntl is None:
        fail(
            "Reliable plaintext handoff locking is only supported on POSIX systems.",
            12,
        )
    descriptor = None
    try:
...
def quarantine_claim(claimed: pathlib.Path, agent_id: str) -> None:
    ...
    failed = (
        claimed.parent
        / f"{AGENT_TYPE}.failed.{safe_agent_id}.{uuid.uuid4().hex}.json"
    )
    try:
...
def stage_locked(
    root: pathlib.Path, ttl_seconds: int, assignment: str
) -> Tuple[dict, pathlib.Path]:
    pending = root / f"{AGENT_TYPE}.pending.json"
    now = datetime.datetime.now(datetime.timezone.utc)
    replace_expired = False
    reconcile_claims(root, now)
    if any(root.glob(f"{AGENT_TYPE}.claimed.*.json")) or any(
        root.glob(f"{AGENT_TYPE}.failed.*.json")
    ):
        fail(
            "A zai_glm53_worker handoff is already claimed or quarantined. "
    ...
    if pending.exists():
        try:
            ...
            fail("The existing GLM-5.3 handoff is malformed.", 9)
        if existing is not None:
            try:
                _, expires_at = validate_envelope(existing)
            except EnvelopeError:
                fail("The existing GLM-5.3 handoff has an invalid schema.", 9)
            if expires_at > now:
                fail(
                    "A zai_glm53_worker handoff is already pending. "
                    "Let it be consumed or expire before staging another.",
                    3,
                )
    ...
    try:
        ...
        if replace_expired:
            ...
            try:
                ...
                fail(
                    "A zai_glm53_worker handoff is already pending. "
                    "Consume or remove it before staging another.",
                    3,
                )
            except OSError as error:
...
def stage(root: pathlib.Path, ttl_seconds: int) -> None:
    ...
    if not assignment.strip():
        fail("Refusing to stage an empty GLM-5.3 assignment.", 2)
...
def run_target_hook_locked(root: pathlib.Path, hook_input: dict) -> None:
    ...
    if any(root.glob(f"{AGENT_TYPE}.claimed.*.json")) or any(
        ...
        fail(
            "A plaintext handoff is already claimed or quarantined for a "
            "zai_glm53_worker.",
            11,
        )
    if not pending.exists():
        fail("No plaintext handoff was available for the zai_glm53_worker start.", 10)
    agent_id = re.sub(
        r"[^A-Za-z0-9_-]",
        "_",
        str(hook_input.get("agent_id") or uuid.uuid4().hex),
    )
    ...
    try:
        ...
        quarantine_claim(claimed, agent_id)
        fail("The pending GLM-5.3 handoff is malformed or invalid.", 5)
    if expires_at <= now:
        try:
            claimed.unlink()
        except OSError as error:
            transport_failure("removing an expired pending handoff", error)
        fail("The pending GLM-5.3 handoff expired before the child started.", 6)
    assignment = envelope["assignment"]
    additional_context = (
        "You are the spawned zai_glm53_worker child, not the root agent. "
        "The parent supplied the complete task below through a one-time plaintext "
        "handoff because provider-internal collaboration ciphertext is not a "
        "reliable cross-provider task carrier. Treat this as the task contract. "
        "Do not continue the parent's unrelated work and do not report the "
        "assignment missing merely because the encrypted collaboration payload "
        "is unreadable.\n\n"
...
def run_hook(root: pathlib.Path) -> None:
    ...
    if (
        ...
        or hook_input.get("agent_type") != AGENT_TYPE
    ):
        return
    with state_lock(root):
